...
local function runAimAnimation(context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    local aim_proc = context:getAimingProgress()
    if (((aim_proc - 0.01) > last_aim_prog) or (aim_proc == 1)) then
        context:runAnimation("draw_aim", track, false, PLAY_ONCE_STOP, 0.1)
        context:stopAnimation(track)
        context:adjustAnimationProgress(track, aim_proc, true)
    elseif ((aim_proc + 0.01) < last_aim_prog) then
        context:runAnimation("put_away_aim", track, false, PLAY_ONCE_STOP, 0.1)
        context:stopAnimation(track)
        context:adjustAnimationProgress(track, 1 - aim_proc, false)
    end
    last_aim_prog = aim_proc
end
...
function main_track_states.start.transition(this, context, input)
    ...
    if (input == INPUT_DRAW) then
        ...
        if (isNoAmmo(context) == false) then
...
function main_track_states.idle.transition(this, context, input)
    ...
    if ((context:getAimingProgress() > 0) and (isNoAmmo(context) == false)) then
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT and context:getAimingProgress() > 0.98) then
